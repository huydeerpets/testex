...
function oldPluginCode() {
  ...
  PostMenuComponent.registerButton(function(visibleButtons){
    ...
    if (canAccept) {
      visibleButtons.splice(position,0,new Button('acceptAnswer', 'expired.accept_answer', 'check-square-o', {className: 'unaccepted'}));
    }
    if (canUnaccept || accepted) {
      var locale = canUnaccept ? 'expired.unaccept_answer' : 'expired.accepted_answer';
  ...
  api.addPostMenuButton('expired', attrs => {
    ...
    if (canAccept) {
      return {
        ...
        title: 'expired.accept_answer',
        position
      };
    } else if (canUnaccept || accepted) {
      const title = canUnaccept ? 'expired.unaccept_answer' : 'expired.accepted_answer';
      return {
